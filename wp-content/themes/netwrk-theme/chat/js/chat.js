...
var originalTitle;
var blinkOrder = 0;
//var serverRoot = location.protocol + '//' + location.host+'/coregen/netwrkdemo';
var serverRoot = 'http://netwrkdemo.coregensolutions.com';
...
function chatWith(chatuser, display_name) {
	createChatBox(chatuser, display_name);
	jQuery("#chatbox_"+chatuser+" .chatboxtextarea").focus();
}

function createChatBox(chatboxtitle, display_name, minimizeChatBox) {
	...
	.addClass("chatbox")
	.html('<div class="chatboxhead"><div class="chatboxtitle">'+display_name+'</div><div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">-</a> <a href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">X</a></div><br clear="all"/></div><div class="chatboxcontent"></div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div>')
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: serverRoot+"/process-chat?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
                ...
                jQuery.ajax({
                    ...
                    success: function(response){
        				...
        				if (item.s == 2) {
        					...
        					jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+display_name+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+item.m+'</span></div>');
        				}
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post(serverRoot+"/process-chat?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post(serverRoot+"/process-chat?action=sendchat", {to: chatboxtitle, message: message} , function(data){
                ...
                jQuery.ajax({
                    ...
                    success: function(response){
                        var display_name = response;
				        jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+display_name+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+message+'</span></div>');
				        jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
                    }
                });
			});
...
function startChatSession(){  
	jQuery.ajax({
	  url: serverRoot+"/process-chat?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
			...
			if (item)	{ // fix strange ie bug
                ...
                jQuery.ajax({
                    ...
                    success: function(response){
                        var display_name = response;
        				if (jQuery("#chatbox_"+chatboxtitle).length <= 0) {
        					createChatBox(chatboxtitle, display_name, 1);
        				}
